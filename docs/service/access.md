...
By default, when an item is uploaded to the service either through the manual upload through the Web UI or through the API methods `publish` and `publish_data`, the item is private to the item's owner.  It can be explicitly shared with other users either through the Web UI or through an API call.
...
Clicking the checkbox  next to a user shares the item with that user.  Hitting 'Share' brings the user back to the view screen with the share list updated; clicking cancel leaves the share list unchanged.

#### Sharing  Through the API
An item is shared in the API at the time it is published, either through the `publish` method for dashboards or the `publish_data` method for tables.  An optional parameter to the body payload, `share_list` is the list of users with whom the item should be shared.  So, for example,
```
...
```
Will publish the nightingale.sdml table and share it with users foo@bar.com, fred@fake-domain.com
...
#### Special Users
There are two distinguished users of the Service, which correspond to groups of users:
- HUB: All users of the HUB.  Sharing with the HUB is the way that items are published to all Hub users.
- PUBLIC: Everyone.  Sharing with the PUBLIC user is publication.

### Publication
Publication of an item is just sharing it with the PUBLIC user.