...
| get_filtered_rows	| POST |	- table, the name of the table <br>-  Optional: columns, a list of columns to get, <br> -  Optional: filter_spec, a filter in SDQL format |	A list of rows which match the filter, or all rows if the filter is not present|  Yes |
